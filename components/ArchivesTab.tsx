...
import { Pencil, Trash2, Archive, MapPin, StickyNote, TrendingUp, TrendingDown, AlertCircle, Check, Loader2, Sparkles } from 'lucide-react'
...
export default function ArchivesTab({ items, roiTarget, onEdit, onDelete, onDetail, onPatchSalePrice }: ArchivesTabProps) {
  ...
  // Hits vendus sans prix → section récupération
  const orphanHits = items
    .filter((i) => i.is_hit && i.status === 'Vendu')
    .sort((a, b) => new Date(b.sold_at ?? b.created_at).getTime() - new Date(a.sold_at ?? a.created_at).getTime())
  ...
  if (soldItems.length === 0 && orphanHits.length === 0) return <EmptyArchives />

  // Pour les lots, le revenu = revenue_generated (somme cumulée ventes réelles)
  const totalRevenue  = soldItems.reduce((s, i) => s + (i.is_lot ? (i.revenue_generated ?? 0) : (i.actual_sale_price ?? 0)), 0)
  ...
  // ROI global : dénominateur = prix d'achat seul (formule : (Vente - (Achat+Frais+Boost)) / Achat)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Récupération : hits vendus sans prix ── */}
      {orphanHits.length > 0 && (
        <div className="bg-amber-500/5 border border-amber-500/25 rounded-2xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            <AlertCircle size={13} className="text-amber-400" />
            <p className="text-xs font-semibold text-amber-400">
              {orphanHits.length} hit{orphanHits.length > 1 ? 's' : ''} vendu{orphanHits.length > 1 ? 's' : ''} — prix de vente manquant
            </p>
          </div>
          <div className="space-y-2">
            {orphanHits.map((hit) => (
              <div key={hit.id} className="flex items-center gap-3 bg-zinc-900/60 rounded-xl px-3 py-2.5 border border-zinc-800">
                ...
                <p className="text-xs font-medium text-white flex-1 truncate">{hit.pokemon_name ?? hit.item_name}</p>
                {hit.card_number && <span className="text-[10px] text-zinc-600 shrink-0">#{hit.card_number}</span>}
                {hit.actual_sale_price != null ? (
                  <span className="text-xs text-emerald-400 font-semibold shrink-0">{formatCurrency(hit.actual_sale_price)} ✓</span>
                ) : (
        ...
        <>
                      ...
                      <tr key={item.id} onClick={() => onDetail(item)} className="cursor-pointer hover:bg-zinc-800/30 transition-colors group">
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <p className="text-white font-semibold">
                            {item.is_lot
                              ? (item.revenue_generated != null ? formatCurrency(item.revenue_generated) : '—')
                              : (item.actual_sale_price != null ? formatCurrency(item.actual_sale_price) : '—')
                            }
                          </p>
                          {item.is_lot && item.item_count && item.items_sold != null && (
          ...
          <div className="flex items-center gap-3 px-1">
            ...
            <span className="text-[11px] text-zinc-600">
              Chez Romain : {formatCurrency(soldItems.filter(i => i.location === 'Chez Romain').reduce((s, i) => s + (i.is_lot ? (i.revenue_generated ?? 0) : (i.actual_sale_price ?? 0)), 0))} ·
              Chez Célian : {formatCurrency(soldItems.filter(i => i.location === 'Chez Célian').reduce((s, i) => s + (i.is_lot ? (i.revenue_generated ?? 0) : (i.actual_sale_price ?? 0)), 0))}
            </span>
          </div>
        </>
      )}
